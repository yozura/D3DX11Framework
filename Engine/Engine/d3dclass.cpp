...
bool D3DClass::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hWnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	/* 1. 그래픽 카드 인터페이스 정보 얻어오기 (주사율의 분모, 분자, 그래픽 카드의 이름, 메모리 크기 등) */

	// DirectX 그래픽 인터페이스 팩토리를 만듭니다.
	result = CreateDXGIFactory(__uuidof(IDXGIFactory), (void**)&factory);
	if (FAILED(result)) return false;

	// 팩토리 객체를 사용하여 첫 번째 그래픽 카드 인터페이스에 대한 어댑터를 만듭니다.
	result = factory->EnumAdapters(0, &adapter);
	if (FAILED(result)) return false;

	// 출력(모니터)에 대한 첫 번째 어댑터를 나열합니다.
	...
	// 백버퍼로 일반적인 32bit 서페이스를 지정합니다.
	swapChainDesc.BufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;

	// 백버퍼의 새로고침 비율을 설정합니다.
	// 수직동기화 설정이 켜져있으면 시스템에서 정한 주사율을 사용하고
	// 꺼져있으면 최대한 빠르게 그릴 것입니다.
	if (m_vsync_enabled)
	{
	...
	// 스왑체인, Direct3D 디바이스, Direct3D 디바이스 컨텍스트를 생성합니다.
	...
	// 렌더 타겟 뷰와 깊이-스텐실 버퍼를 각각 출력 파이프라인에 바인딩합니다.
	m_deviceContext->OMSetRenderTargets(1, &m_renderTargetView, m_depthStencilView);

	/* 7. 래스터라이저 설정 */
	
	// 어떤 도형을 어떻게 그릴 것인지 결정하는 래스터라이저 description을 작성합니다.
	...
	// 작성한 description으로부터 래스터라이저 상태를 생성합니다.
	...
	// 2D 렌더링 깊이 버퍼 스텐실 상태 description 작성
...
{
	// 먼저 스왑 체인을 윈도우 모드로 바꾸는 함수를 호출한다.
	// 풀스크린 모드에서 스왑 체인을 해제하면 몇가지 예외가 발생하므로... 이를 피하기 위해	
